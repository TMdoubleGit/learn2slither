...
class Displayer:
    ...
    def display(self, action, state, reward, prev_state =[], is_new_state = False):
        ...
        output.append(f"FULL BOARD, SNAKE VISION AND STATE AT {state_tempo}\n".center(120))
        output.append("-" * 120)
        ...
        for i in range(height):
            ...
            if len(state) > height:
                extra_state_info = f"{state_labels[height + i]}: {state[height + i]:.2f}" if i < len(state) - height else ""
            if len(state) > 2 * height:
                extra_state_info2 = f"{state_labels[2 * height + i]}: {state[2 * height + i]:.2f}" if i < len(state) - 2 * height else ""
            if len(state) > 3 * height:
                extra_state_info3= f"{state_labels[3 * height + i]}: {state[3 * height + i]:.2f}" if i < len(state) - 3 * height else ""
            output.append(fb_row.ljust(23) + "     " + vis_row.ljust(23) + "     " + state_info.ljust(23) + "     " + extra_state_info.ljust(23)+ "     " + extra_state_info2.ljust(23)+ "     " + extra_state_info3.ljust(23))
